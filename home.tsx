...
export default function Home() {
  ...
  const loadStoresFromLocalStorage = (): Store[] => {
    try {
      ...
      if (cachedStores) {
        ...
        if (!hasTotal) {
          // Asignar un projectId por defecto, por ejemplo "1" o el ID del primer proyecto si está disponible
          // Nota: activeProjectId podría no estar disponible aquí en la carga inicial, se manejará en el useEffect principal.
          stores.unshift({ id: "total", name: "Total", projectId: "1" })
        }
        return stores
      }
    } catch (error) {
    ...
    return [{ id: "total", name: "Total", projectId: "1" }]
  }
  ...
  // 1. useEffect para la carga inicial de proyectos y establecer el proyecto activo
  useEffect(() => {
    const loadInitialProjectsAndSetId = async () => {
      if (user) {
        ...
        try {
          ...
          if (projects && projects.length > 0) {
            ...
            if (projectToSet && activeProjectId !== projectToSet.id) {
              ...
              console.log("Proyecto activo inicial establecido:", projectToSet.id)
            } else if (!projectToSet && activeProjectId !== "") {
              setActiveProjectId("")
              console.log("No hay proyectos, limpiando proyecto activo.")
            }
          } else {
            setProjects([])
            setActiveProjectId("") // No se encontraron proyectos
          }
        } catch (error) {
    ...
    loadInitialProjectsAndSetId()
  }, [user]) // Solo depende del usuario

  // 2. useEffect para cargar tiendas y productos cuando el proyecto activo cambia
  useEffect(() => {
      ...
      if (user && activeProjectId) {
        ...
        try {
          ...
          if (totalStore) {
            ...
            setActiveStoreId("") // No hay tiendas disponibles para este proyecto
          }
          ...
          setErrorMessage("Error al cargar datos del proyecto. Por favor, inténtalo de nuevo.")
        } finally {
          setIsLoading(false)
        }
      } else if (user && !activeProjectId) {
        // Si el usuario existe pero no hay activeProjectId (ej. no hay proyectos disponibles o el proyecto fue eliminado)
        // Limpiar tiendas y productos, asegurando que una tienda "Total" por defecto esté presente
        console.log("Usuario presente pero sin proyecto activo, limpiando datos y estableciendo tienda 'Total'.")
        setStores([{ id: "total", name: "Total", projectId: "1" }]) // Tienda "Total" por defecto
  ...
  }, [user, activeProjectId]) // Depende del usuario y del proyecto activo
