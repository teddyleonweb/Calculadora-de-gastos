...
export default function Home() {
  ...
  async function handleUpdateProduct(productId: string, title: string, price: number, quantity: number) {
    if (!user) return

    try {
      setIsLoading(true)
      console.log("Actualizando producto:", productId, title, price, quantity)
      ...
      if (updateSuccess) {
        ...
        setProducts((prevProducts) => {
          const updatedProducts = prevProducts.map((product) =>
            product.id === productId ? { ...product, title, price, quantity } : product,
          )
          saveProductsToLocalStorage(updatedProducts)
          return updatedProducts
        })
