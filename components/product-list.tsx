...
// Memoizar el componente ProductItem para evitar renderizaciones innecesarias
          ...
          {product.image && (
            <div className="sm:w-1/4 md:w-1/5 p-2 flex items-center justify-center bg-gray-50">
              <ImageWithFallback
                ...
                onClick={() => product.image && onOpenImageModal(product.image)}
              />
            </div>
          )}
          ...
          <div className={`p-3 space-y-3 flex-grow ${product.image ? "sm:w-3/4 md:w-4/5" : "w-full"}`}>
            <div>
              ...
              <input
                type="text"
                value={editTitle}
                onChange={(e) => onStartEditing({ ...product, editTitle: e.target.value })}
              ...
              <div className="flex-1 min-w-[120px]">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    onStartEditing({ ...product, editPrice: filteredValue })
                  }}
              ...
              <div className="w-24">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    const value = e.target.value
                    const filteredValue = value.replace(/[^0-9]/g, "")
                    onStartEditing({ ...product, editQuantity: filteredValue })
                  }}
          ...
          {product.image && (
            <div className="sm:w-1/4 md:w-1/5 p-2 flex items-center justify-center bg-gray-50">
              <ImageWithFallback
                ...
                onClick={() => onOpenImageModal(product.image)}
...
export default function ProductList({
  ...
  // Filtrar productos por tienda activa - memoizado para evitar recálculos
  ...
  const handleProductEdit = useCallback((product: any) => {
    if (product.editTitle !== undefined) {
      setEditTitle(product.editTitle)
    } else if (product.editPrice !== undefined) {
      setEditPrice(product.editPrice)
    } else if (product.editQuantity !== undefined) {
      setEditQuantity(product.editQuantity)
    } else {
  ...
  const saveEditing = useCallback(
    ...
    [editTitle, editPrice, editQuantity, onUpdateProduct],
  )
  ...
  const openImageModal = useCallback((imageSrc: string | undefined) => {
    if (imageSrc) {
    ...
    <div className="grid grid-cols-1 gap-4">
      ...
      {filteredProducts.map((product) => (
        <ProductItem
          ...
          onStartEditing={handleProductEdit}
